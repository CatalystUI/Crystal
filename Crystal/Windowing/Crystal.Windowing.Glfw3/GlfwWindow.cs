...
namespace Crystal.Windowing.Glfw3 {
    ...
    public class GlfwWindow : ICrystalWindow {
        ...
        private unsafe void HandleWindowSize(WindowHandle* handle, int width, int height) {
            OnResized();
        }
        
        /// <inheritdoc cref="GlfwCallbacks.WindowPosCallback"/>
        private unsafe void HandleWindowPos(WindowHandle* handle, int x, int y) {
            RefreshProperties();
            OnRepositioned();
        }
        
        /// <inheritdoc cref="GlfwCallbacks.WindowMaximizeCallback"/>
        private unsafe void HandleWindowMaximize(WindowHandle* handle, bool maximized) {
            RefreshProperties();
            if (maximized) OnMaximized();
        }
        
        /// <inheritdoc cref="GlfwCallbacks.WindowIconifyCallback"/>
        private unsafe void HandleWindowIconify(WindowHandle* handle, bool iconified) {
            RefreshProperties();
            if (iconified) OnMinimized();
        }
        
        /// <inheritdoc cref="GlfwCallbacks.WindowFocusCallback"/>
        private unsafe void HandleWindowFocus(WindowHandle* handle, bool focused) {
            RefreshProperties();
        ...
        private unsafe void HandleMonitorCallback(Silk.NET.GLFW.Monitor* monitor, ConnectedState state) {
            // Propagate the event
